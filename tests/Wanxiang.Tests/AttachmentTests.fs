...
let ``断线清理：AbortByConnection 删除该连接未完成上传的临时文件`` () =
    ...
    try
        ...
        store.AppendChunk(aid, Convert.ToBase64String payload[0..3]) |> function Ok () -> () | Error e -> failwith (WanxiangError.message e)
        ...
        store.AppendChunk(aid2, Convert.ToBase64String payload[0..3]) |> function Ok () -> () | Error e -> failwith (WanxiangError.message e)
        ...
        store.AppendChunk(aid2, Convert.ToBase64String payload[4..]) |> function Ok () -> () | Error e -> failwith (WanxiangError.message e)
        store.Complete(aid2, hash) |> function Ok _ -> () | Error e -> failwith (WanxiangError.message e)
        store.Dispose()
    finally
        cleanup dir
