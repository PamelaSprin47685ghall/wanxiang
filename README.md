...
- **批量管理**：会话右键 →「多选」，之后点行即切换选中，底部条可整批置顶 / 归档 / 删除
- **置顶 / 归档 / 导出**：会话右键
- **切模型**：输入框左下角，按服务商分组；`Ctrl` `M` 直接打开选择器
...
| `Ctrl` `,` | 打开设置 |
| `Ctrl` `M` | 切换模型 |
| `Ctrl` `↑` / `↓` | 跳到上一条 / 下一条提问（顶栏同款按钮） |
| `Ctrl` `L` | 聚焦消息输入框 |
| `Ctrl` `/` | 快捷键列表 |
| `Ctrl` `Enter` | 设置表单保存（服务商 / MCP / 会话参数 / 生成设置） |
| `Enter` | 发送（可在设置里改为 `Ctrl` `Enter`） |
| `Shift` `Enter` | 换行 |
| `P` / `F2` / `Delete` | 置顶 / 重命名 / 删除当前会话（会话行聚焦时） |
| `Esc` | 关闭弹层 / 取消编辑 |
