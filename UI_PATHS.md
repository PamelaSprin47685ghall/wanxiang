...
- Composer 历史提示词回溯：支持 Up/Down 方向键（无修饰键时）智能召回已发提示词历史，保留未提交草稿，不干扰 Shift+Up/Down 文本选择。
- Markdown 多列表格平滑横向滚动：>4 列宽表格采用 MinWidth 120.0 + ScrollViewer 局部横向滚动契约，保持字号与换行排版，彻底避免宽表挤碎成单字符窄列。
- 聊天滚动到底与未读计数胶囊：向上阅读时流式追加新消息动态显示带未读数的墨蓝微胶囊，点击缓动滚动到底，切会话或到顶自动清零复位。
- 侧栏搜索键盘走查：搜索框按 Down 直达首个会话，Enter 打开结果，Escape 清空或恢复焦点。
